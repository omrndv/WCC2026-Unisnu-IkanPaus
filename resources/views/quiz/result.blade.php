...
<section class="min-h-screen bg-primary relative overflow-hidden flex items-center py-32">
    ...
    <div class="container mx-auto px-6 relative z-10 max-w-xl text-center">
        ...
        {{-- Emoji besar --}}
        <div class="text-8xl mb-6 animate-bounce">{{ $rekomendasi['emoji'] }}</div>
        ...
        {{-- Nama varian --}}
        <h1 class="text-3xl md:text-5xl font-title text-white mb-6 leading-tight">
            {{ $rekomendasi['varian'] }}
        </h1>

        {{-- Deskripsi --}}
        <div class="bg-white/10 backdrop-blur-md rounded-[2rem] p-8 border border-white/10 shadow-2xl mb-10">
        ...
        <div class="flex flex-col sm:flex-row justify-center gap-4">
            {{-- Pesan via WA dengan nama varian terisi otomatis --}}
            @php
                $pesanWa = urlencode("Halo, saya baru selesai Flavor Finder Quiz dan direkomendasikan *{$rekomendasi['varian']}*. Apakah masih tersedia? Terima kasih!");
            @endphp
            <a href="https://wa.me/62{{ e(config('site.wa_number', '')) }}?text={{ $pesanWa }}"
        ...
        <a href="/" class="inline-block mt-8 text-white/50 hover:text-white transition text-sm">
            ← Kembali ke Beranda
        </a>
    </div>
</section>
