        ...
        <div class="flex flex-col lg:flex-row gap-10">
                ...
                <div id="hampers-builder"
                    ...
                    <div class="mb-8">
                        <div class="flex justify-between items-end mb-4">
                            <h4 class="font-bold text-primary text-sm uppercase tracking-wider">2. Isi Varian (Maks. 3)</h4>
                            <span class="text-xs text-gray-400">Pilih <span id="isian-count">0</span>/3</span>
                        </div>
                        <div class="space-y-3">
                            @foreach($produk as $p)
                            <label class="flex items-center justify-between p-3 border border-gray-100 rounded-xl hover:bg-gray-50 cursor-pointer transition-colors">
                                <div class="flex items-center gap-3">
                                    <input type="checkbox"
                                        ...
                                        data-nama="{{ $p->nama }}"
                                        data-harga="{{ $p->harga }}">
                                    <span class="font-medium text-primary">{{ $p->nama }}</span>
                                </div>
                                <span class="text-sm font-bold text-gray-500">Rp {{ number_format($p->harga, 0, ',', '.') }}</span>
                            </label>
                            @endforeach
                        </div>
                    </div>
            ...
            <div class="w-full lg:w-5/12 order-1 lg:order-2">
                <div
                    ...
                    <div class="space-y-4 mb-6 min-h-[150px]" id="order-summary-items">
                        <div class="reguler-item" id="reguler-default">
                            @if($produk->isNotEmpty())
                            <div class="flex justify-between items-center">
                                <div class="flex items-center gap-3">
                                    <div class="w-12 h-12 rounded-lg bg-gray-100 overflow-hidden">
                                        @if($produk->first()->gambar)
                                        <img src="{{ asset('storage/' . $produk->first()->gambar) }}" class="w-full h-full object-cover">
                                        @else
                                        <div class="w-full h-full bg-bgLight/40"></div>
                                        @endif
                                    </div>
                                    <div>
                                        <h5 class="font-bold text-primary text-sm">{{ $produk->first()->nama }}</h5>
                                        <p class="text-xs text-gray-500">1 x Rp {{ number_format($produk->first()->harga, 0, ',', '.') }}</p>
                                    </div>
                                </div>
                                <span class="font-bold text-primary text-sm">Rp {{ number_format($produk->first()->harga, 0, ',', '.') }}</span>
                            </div>
                            @endif
                        </div>
                            ...
                            <div class="bg-highlight/10 border border-highlight/20 rounded-xl p-4">
                                <div class="flex justify-between items-start mb-2">
                                    <h5 class="font-bold text-primary text-sm flex items-center gap-2">
                                        <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 text-highlight" viewBox="0 0 20 20" fill="currentColor">
                                            <path fill-rule="evenodd" d="M5 5a3 3 0 015-2.236A3 3 0 0114.83 6H16a2 2 0 110 4h-5V9a1 1 0 10-2 0v1H4a2 2 0 110-4h1.17C5.06 5.687 5 5.35 5 5zm4 1V5a1 1 0 10-1 1h1zm3 0a1 1 0 10-1-1v1h1z" clip-rule="evenodd" />
                                            <path d="M9 11H3v5a2 2 0 002 2h4v-7zM11 18h4a2 2 0 002-2v-5h-6v7z" />
                                        </svg>
                                        Custom Hampers
                                    </h5>
                                    <span class="font-bold text-primary text-sm" id="hampers-total-badge">Rp 0</span>
                                </div>
                                <ul class="text-xs text-gray-500 space-y-1 list-disc list-inside" id="hampers-detail-list">
                                    <li>Belum ada varian dipilih</li>
                                </ul>
                            </div>
                        </div>
                    </div>
...
<script>
    ...
    function toggleOrderType() {
        ...
        const regulerDefault = document.getElementById('reguler-default');
        ...
        if (orderType === 'hampers') {
            ...
            regulerDefault.style.display = 'none';
            ...
            gsap.to(hampersBuilder, {
                ...
                duration: 0.4,
                ease: "power2.in",
                onComplete: () => hampersBuilder.classList.add('hidden')
            });
            stepNumber.textContent = "2";
            regulerDefault.style.display = 'block';
            hampersSummary.classList.add('hidden');
        }
    }

    // Update ringkasan hampers saat checkbox berubah
    ...
    isianCheckboxes.forEach(checkbox => {
        checkbox.addEventListener('change', function() {
            const checked = document.querySelectorAll('.custom-isian:checked');
            if (checked.length > 3) {
                this.checked = false;
                alert("Maksimal hanya bisa memilih 3 varian rasa untuk satu hampers ya!");
                return;
            }
            isianCountText.textContent = checked.length;

            // Update detail list
            const list = document.getElementById('hampers-detail-list');
            list.innerHTML = checked.length === 0 ?
                '<li>Belum ada varian dipilih</li>' :
                [...checked].map(c => `<li>${c.dataset.nama}</li>`).join('');

            // Update total hampers
            const packagingPrice = document.querySelector('input[name="packaging"]:checked')?.value === 'premium' ? 25000 : 15000;
            const isianTotal = [...checked].reduce((sum, c) => sum + parseInt(c.dataset.harga), 0);
            const total = packagingPrice + isianTotal;
            document.getElementById('hampers-total-badge').textContent = 'Rp ' + total.toLocaleString('id-ID');
        });
    });

    function kirimWA() {
        const nama = document.querySelector('input[placeholder="Cth: Bagus Purnama"]').value;
        const noWa = document.querySelector('#wa-input').value;
        const alamat = document.querySelector('textarea[placeholder="Masukkan alamat lengkap pengiriman..."]').value;
        const tipe = document.querySelector('input[name="order_type"]:checked').value;
        ...
        let pesan = `*Pesanan Getuk Asri*\n\n`;
        pesan += `Nama: ${nama}\nNo. WA: ${noWa}\nAlamat: ${alamat || '-'}\n\n`;

        if (tipe === 'hampers') {
            const checked = document.querySelectorAll('.custom-isian:checked');
            const packaging = document.querySelector('input[name="packaging"]:checked')?.value || 'besek';
            const ucapan = document.querySelector('textarea[placeholder="Tulis ucapan untuk penerima..."]').value;
            const varian = [...checked].map(c => c.dataset.nama).join(', ');

            pesan += `Tipe: Custom Hampers\nKemasan: ${packaging}\nVarian: ${varian || '-'}\n`;
            if (ucapan) pesan += `Ucapan: ${ucapan}\n`;
        } else {
            pesan += `Tipe: Pesanan Reguler\n`;
        }
