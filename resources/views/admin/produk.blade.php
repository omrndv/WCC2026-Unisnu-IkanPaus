...
<div class="mb-10 flex justify-between items-end">
    <div>
        ...
        <h2 class="text-2xl font-title font-bold text-primary italic text-highlight">Kelola Cita Rasa</h2>
    </div>
...
<div class="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
    ...
    <div class="bg-white p-6 rounded-[2.5rem] border border-gray-100 shadow-sm hover:shadow-xl transition-all duration-500 group relative">
        ...
        <div class="h-52 bg-bgLight rounded-[2rem] mb-6 overflow-hidden relative">
            <img src="{{ $item->gambar ? asset('storage/' . $item->gambar) : 'https://images.unsplash.com/photo-1505935428862-770b6f24f629?auto=format&fit=crop&q=80&w=800' }}" class="w-full h-full object-cover group-hover:scale-110 transition-transform duration-700">
        </div>

        <div class="space-y-1">
            <h4 class="font-title font-bold text-xl text-primary italic">{{ $item->nama }}</h4>
            <p class="text-secondary font-bold text-lg tracking-tight">Rp {{ number_format($item->harga, 0, ',', '.') }}</p>
        </div>

        <div class="mt-6 pt-6 border-t border-gray-50 flex gap-3">
            {{-- Tambahin ID di parameter pertama --}}
            <button onclick="editProduct({{ $item->id }}, '{{ $item->nama }}', {{ $item->harga }}, '{{ $item->status }}', '{{ $item->deskripsi }}', '{{ $item->gambar ? asset('storage/' . $item->gambar) : 'https://images.unsplash.com/photo-1505935428862-770b6f24f629?auto=format&fit=crop&q=80&w=800' }}')"
            ...
            <button onclick="confirmDeleteProduct({{ $item->id }}, '{{ $item->nama }}')" class="text-[10px] font-bold uppercase tracking-widest text-red-400 border border-red-50/50 px-4 py-3 rounded-full hover:bg-red-500 hover:text-white transition-all duration-300">
...
<div id="modalTambahProduk" class="hidden fixed inset-0 z-[99] bg-primary/40 backdrop-blur-sm flex items-center justify-center p-6">
    <form action="{{ route('admin.produk.store') }}" method="POST" enctype="multipart/form-data" class="bg-white w-full max-w-4xl rounded-[3rem] shadow-2xl overflow-hidden flex flex-col md:flex-row">
        @csrf
        ...
        <div class="md:flex-1 p-10 space-y-6">
            <div class="flex justify-between items-center mb-2">
                ...
                <button type="button" onclick="document.getElementById('modalTambahProduk').classList.add('hidden')" class="text-gray-400 hover:text-red-500 transition-colors text-2xl">&times;</button>
            </div>

            <div class="grid gap-5">
                <div class="space-y-1">
                    <label class="text-[9px] font-bold uppercase text-gray-400 ml-4">Nama Varian</label>
                    <input type="text" name="nama" required placeholder="Misal: Getuk Keju Lumer" class="w-full px-6 py-3 rounded-full bg-bgLight/20 border border-gray-100 outline-none focus:ring-2 focus:ring-highlight text-sm font-bold">
                </div>
                <div class="grid grid-cols-2 gap-4">
                    <div class="space-y-1">
                        <label class="text-[9px] font-bold uppercase text-gray-400 ml-4">Harga (Rp)</label>
                        <input type="number" name="harga" required placeholder="25000" class="w-full px-6 py-3 rounded-full bg-bgLight/20 border border-gray-100 outline-none text-sm font-bold text-secondary">
                    </div>
                    <div class="space-y-1">
                        <label class="text-[9px] font-bold uppercase text-gray-400 ml-4">Status Stok</label>
                        <select name="status" class="w-full px-6 py-3 rounded-full bg-bgLight/20 border border-gray-100 outline-none text-xs font-bold text-accent">
                ...
                <div class="space-y-1">
                    ...
                    <textarea name="deskripsi" rows="3" placeholder="Jelaskan keunikan rasa varian ini..." class="w-full px-6 py-4 rounded-[1.5rem] bg-bgLight/20 border border-gray-100 outline-none text-xs italic"></textarea>
                </div>
            </div>

            <button type="submit" class="w-full bg-primary text-highlight py-4 rounded-full font-bold uppercase tracking-[0.2em] text-[10px] shadow-xl hover:bg-secondary transition-all">Simpan ke Katalog</button>
        </div>
    </form>
</div>
...
<div id="modalEditProduk" class="hidden fixed inset-0 z-[99] bg-primary/40 backdrop-blur-sm flex items-center justify-center p-6">
    <form id="formEditProduk" method="POST" enctype="multipart/form-data" class="bg-white w-full max-w-4xl rounded-[3rem] shadow-2xl overflow-hidden flex flex-col md:flex-row">
        @csrf
        @method('PUT')
        <div class="md:w-1/3 bg-bgLight/30 p-10 border-r border-gray-100 flex flex-col items-center justify-center text-center">
            ...
            <div class="w-full aspect-square rounded-[2rem] border-2 border-highlight bg-white overflow-hidden relative group">
                ...
                <input type="file" name="gambar" id="editImageInput" class="absolute inset-0 opacity-0 cursor-pointer" accept="image/*">
        ...
        <div class="md:flex-1 p-10 space-y-6">
            <div class="flex justify-between items-center mb-2">
                ...
                <button type="button" onclick="document.getElementById('modalEditProduk').classList.add('hidden')" class="text-gray-400 hover:text-red-500 transition-colors text-2xl">&times;</button>
            </div>

            <div class="grid gap-5">
                <div class="space-y-1">
                    <label class="text-[9px] font-bold uppercase text-gray-400 ml-4">Nama Varian</label>
                    <input type="text" name="nama" id="editNama" class="w-full px-6 py-3 rounded-full bg-bgLight/20 border border-gray-100 outline-none focus:ring-2 focus:ring-highlight text-sm font-bold">
                </div>
                <div class="grid grid-cols-2 gap-4">
                    <div class="space-y-1">
                        <label class="text-[9px] font-bold uppercase text-gray-400 ml-4">Harga (Rp)</label>
                        <input type="number" name="harga" id="editHarga" class="w-full px-6 py-3 rounded-full bg-bgLight/20 border border-gray-100 outline-none text-sm font-bold text-secondary">
                    </div>
                    <div class="space-y-1">
                        <label class="text-[9px] font-bold uppercase text-gray-400 ml-4">Status Stok</label>
                        <select name="status" id="editStatus" class="w-full px-6 py-3 rounded-full bg-bgLight/20 border border-gray-100 outline-none text-xs font-bold text-accent">
                            <option value="Tersedia">Tersedia</option>
                            <option value="Habis">Habis</option>
                        </select>
                    </div>
                ...
                <div class="space-y-1">
                    <label class="text-[9px] font-bold uppercase text-gray-400 ml-4">Deskripsi Singkat</label>
                    <textarea name="deskripsi" id="editDeskripsi" rows="3" class="w-full px-6 py-4 rounded-[1.5rem] bg-bgLight/20 border border-gray-100 outline-none text-xs italic"></textarea>
                </div>
            </div>
            ...
            <div class="flex gap-4">
                ...
                <button type="submit" class="flex-[2] bg-primary text-highlight py-4 rounded-full font-bold uppercase tracking-[0.2em] text-[10px] shadow-xl hover:bg-secondary transition-all">Simpan Perubahan</button>
            </div>
        </div>
    </form>
</div>
