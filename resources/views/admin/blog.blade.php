    ...
    <div class="overflow-x-auto">
        <table class="w-full text-left">
            ...
            <tbody class="text-sm divide-y divide-gray-50">
                ...
                @empty
                <tr>
                    <td colspan="5" class="px-8 py-10 text-center text-gray-400 italic">Belum ada cerita yang ditulis.</td>
                </tr>
                @endforelse
            </tbody>
        </table>
...
<div id="modalTambahBlog" class="hidden fixed inset-0 z-[99] bg-primary/40 backdrop-blur-sm flex items-center justify-center p-6">
    <form action="{{ route('admin.blog.store') }}" method="POST" enctype="multipart/form-data" class="bg-white w-full max-w-4xl rounded-[3rem] shadow-2xl overflow-hidden flex flex-col md:flex-row">
        @csrf
        ...
        <div class="md:flex-1 p-10 space-y-6">
            <div class="flex justify-between items-center mb-4">
                ...
                <button type="button" onclick="document.getElementById('modalTambahBlog').classList.add('hidden')" class="text-gray-400 hover:text-red-500 transition-colors">&times; Close</button>
            </div>

            <div class="space-y-4">
                <div class="space-y-1">
                    <label class="text-[9px] font-bold uppercase text-gray-400 ml-4">Judul Artikel</label>
                    <input type="text" name="judul" required placeholder="Contoh: Rahasia Getuk Goreng Renyah..." class="w-full px-6 py-3 rounded-full bg-bgLight/20 border border-gray-100 outline-none focus:ring-2 focus:ring-highlight text-sm font-bold">
                </div>
                <div class="grid grid-cols-2 gap-4">
                    <div class="space-y-1">
                        <label class="text-[9px] font-bold uppercase text-gray-400 ml-4">Kategori</label>
                        <select name="kategori" class="w-full px-6 py-3 rounded-full bg-bgLight/20 border border-gray-100 outline-none text-xs font-bold text-primary">
                ...
                <div class="space-y-1">
                    ...
                    <textarea name="konten" required rows="5" placeholder="Tuliskan cerita menarik di sini..." class="w-full px-6 py-4 rounded-[1.5rem] bg-bgLight/20 border border-gray-100 outline-none focus:ring-2 focus:ring-highlight text-sm italic leading-relaxed"></textarea>
                </div>
            </div>

            <button type="submit" class="w-full bg-primary text-highlight py-4 rounded-full font-bold uppercase tracking-[0.2em] text-[10px] shadow-xl hover:bg-secondary transition-all">Terbitkan Sekarang</button>
        </div>
    </form>
</div>
@endsection
