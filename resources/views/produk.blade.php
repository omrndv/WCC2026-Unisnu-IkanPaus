                ...
                <div class="relative h-80 overflow-hidden img-container bg-gray-100">
                    ...
                    <div class="absolute inset-0 bg-gradient-to-t from-primary/90 via-primary/40 to-transparent opacity-0 group-hover:opacity-100 transition-all duration-500 flex flex-col justify-end p-6 translate-y-4 group-hover:translate-y-0 z-20">
                        <button onclick="openQuickView({{ $p->id }})"
                            class="w-full bg-white text-primary py-3 rounded-full font-bold text-sm hover:bg-highlight hover:text-white transition-all duration-300 mb-3 shadow-lg">
                            Lihat Detail Cepat
                        </button>
...
<script>
    const productsData = {
        ...
        {{ $p->id }}: {
            ...
            badge: '<span class="bg-primary text-white px-3 py-1 rounded-full text-xs font-bold">{{ $p->status }}</span>',
            rating: 5
        },
        @endforeach
    };
