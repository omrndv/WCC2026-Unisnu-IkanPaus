...
@section('title', 'Cerita, Budaya, dan Sejarah Getuk Goreng Sokaraja')
@section('description', 'Baca cerita, budaya, sejarah kuliner, dan tips seputar getuk goreng khas Sokaraja Banyumas dari Getuk Goreng Asri.')
...
<section class="pt-40 pb-20 bg-primary relative overflow-hidden">
    ...
    <div class="container mx-auto px-6 relative z-10 text-center">
        <div class="overflow-hidden mb-4">
            <span
                class="blog-reveal blog-reveal-up inline-block py-1.5 px-5 border border-highlight/50 bg-white/10 backdrop-blur-md text-highlight rounded-full text-xs font-semibold tracking-[0.2em] uppercase mb-4 shadow-lg">
                Sejak Tahun 1990
            </span>
        </div>
        ...
        <div class="overflow-hidden mb-4">
            <span
                class="blog-reveal blog-reveal-up inline-block py-1 px-4 border border-highlight/50 text-highlight rounded-full text-xs font-semibold tracking-widest uppercase">
                Jendela Banyumas
            </span>
        </div>

        <h1 class="blog-reveal blog-reveal-up text-4xl md:text-6xl font-title text-white mb-6">
            Cerita & <span class="italic text-highlight">Budaya</span>
        </h1>

        <p class="blog-reveal blog-reveal-up text-white/70 max-w-2xl mx-auto font-light">
            Menelusuri jejak sejarah getuk goreng dan kearifan lokal masyarakat Sokaraja melalui tulisan.
        </p>
    </div>
</section>
...
<section class="py-20 bg-[#FAFAFA]">
    <div class="container mx-auto px-6">
        ...
        <div class="mb-10 text-gray-500 text-sm italic">
            Menampilkan cerita dalam kategori:
            <span class="text-primary font-bold capitalize">{{ $currentCat }}</span>
        </div>
        @endif

        <div class="grid md:grid-cols-3 gap-12">
            @forelse($blogs as $blog)
            <article class="group blog-reveal blog-reveal-up blog-card">
    ...
    <div class="container mx-auto px-6 relative z-10">
        <div class="bg-white rounded-[3rem] p-12 md:p-20 shadow-2xl flex flex-col md:flex-row items-center gap-12">
            <div class="md:w-1/2 text-center md:text-left">
                <h2 class="text-3xl md:text-4xl font-title text-primary mb-4 leading-tight">
                    Langganan <span class="italic">Kabar Asri</span>
                </h2>
                <p class="text-gray-500 font-light">
                    Dapatkan update cerita budaya, tips kuliner, dan promo eksklusif langsung ke email Anda.
                </p>
            </div>
            ...
            <div class="md:w-1/2 w-full">
                <form class="flex flex-col sm:flex-row gap-4">
                    ...
                    <button
                        class="bg-primary text-white px-10 py-4 rounded-full font-bold hover:bg-secondary transition duration-300 whitespace-nowrap shadow-lg">
                        Daftar Sekarang
                    </button>
                </form>
            </div>
...
<section id="modalReadBlog"
    ...
    <div class="bg-white w-full max-w-6xl rounded-[1.5rem] shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
        <div class="relative min-h-[200px] md:min-h-[280px] bg-primary flex items-end p-6 md:p-10 overflow-hidden shrink-0">
            <div class="absolute inset-0 bg-batik-pattern opacity-10 pointer-events-none z-10"></div>

            <img id="modalBlogCover" src="" alt=""
            ...
            <div class="relative z-20 w-full">
                <span id="modalBlogKategori"
                    class="inline-block py-1 px-3 bg-highlight text-primary rounded-full text-[10px] font-bold uppercase tracking-widest mb-3">
                </span>
        ...
        <div class="p-6 md:p-10 overflow-y-auto text-gray-600 space-y-4 leading-relaxed text-sm md:text-base selection:bg-highlight/30">
            ...
            <div id="modalBlogKonten" class="whitespace-pre-line prose max-w-none text-gray-700"></div>
        </div>
...
<script>
    ...
    function initBlogAnimation() {
        ...
        if (!("IntersectionObserver" in window)) {
            elements.forEach((element) => {
                element.classList.add("blog-visible");
            });
    ...
    function openReadBlogModal(blog, imageUrl, formattedDate) {
        ...
        const title = document.getElementById("modalBlogJudul");
        const category = document.getElementById("modalBlogKategori");
        const date = document.getElementById("modalBlogTanggal");
        const content = document.getElementById("modalBlogKonten");
        const coverImg = document.getElementById("modalBlogCover");

        title.innerText = blog.judul;
        category.innerText = blog.kategori;
        date.innerText = formattedDate;
        content.innerHTML = blog.konten;
